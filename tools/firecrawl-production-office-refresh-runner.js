...
const OUTPUT_DIR = process.env.OUTPUT_DIR || 'firecrawl-output-production-office-refresh';
...
const department = {
  branchId: 'production-office',
  branchName: 'Production Office',
  queryTerms: 'production office coordinator credentials accreditation artist check-in festival production vendor 2026',
  note: 'Supplemental Production Office refresh sweep. Do not overwrite prior Production Office research.'
};
...
const plan = {
  ...
  batches: batchTargets.map(batch => ({ batchId: `branch-research-refresh-${batch.suffix}-production-office`, branchId: department.branchId, branchName: department.branchName, theme: `${department.branchName}: ${batch.theme}`, targets: batch.targets }))
};
