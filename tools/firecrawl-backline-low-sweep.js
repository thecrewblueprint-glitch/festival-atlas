...
const OUTPUT_DIR = process.env.OUTPUT_DIR || 'firecrawl-output-backline';
...
const plan = {
  ...
  batches: [
    { batchId: 'branch-research-batch-001-backline', theme: 'Major anchor festival backline route leads', targets: [
    ...
    { batchId: 'branch-research-batch-002-backline', theme: 'Country, stadium, multi-stage, and camping festival backline route leads', targets: [
    ...
    { batchId: 'branch-research-batch-003-backline', theme: 'DWP and heavy music backline route leads', targets: [
    ...
    { batchId: 'branch-research-batch-004-backline', theme: 'Northeast, EDM, and regional festival backline route leads', targets: [
    ...
    { batchId: 'branch-research-batch-005-backline', theme: 'Regional, coastal, jazz, folk, and specialty backline route leads', targets: [
...
function queryFor(name) {
  return `"${name}" backline vendor instrument tech drum riser guitar bass keyboard DJ gear festival production`;
}
async function researchTarget([opportunityId, opportunityName]) {
  ...
  try {
    const search = await firecrawl('search', { query, limit: MAX_SEARCH_RESULTS, sources: [{ type: 'web' }], country: 'US' }, 'search');
    record.searchResults = searchResults(search).slice(0, MAX_SEARCH_RESULTS);
  } catch (error) {
  ...
  for (const result of candidates) {
    try {
      const scraped = await firecrawl('scrape', { url: result.url, formats: ['markdown'], onlyMainContent: true, removeBase64Images: true, blockAds: true, timeout: 45000, redactPII: true }, 'scrape');
...
function renderMarkdown(results) {
  const lines = [];
  lines.push('# Firecrawl Low-Mode Backline Sweep', '', `Generated: ${results.generatedAt}`, `Estimated credits: ${results.estimatedCredits}`, `Reported credits: ${results.reportedCredits}`, '');
  lines.push('## Public-safety boundary', '', 'Raw/draft research only. Do not publish directly. Review and remove private contacts, phone numbers, personal emails, pay rates, lodging details, rumors, private referrals, NDA information, and client-sensitive information before converting to Production Atlas data.', '');
...
async function main() {
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'firecrawl-backline-low-sweep-results.json'), JSON.stringify(results, null, 2));
  fs.writeFileSync(path.join(OUTPUT_DIR, 'firecrawl-backline-low-sweep-report.md'), renderMarkdown(results));
  fs.writeFileSync(path.join(OUTPUT_DIR, 'firecrawl-backline-low-sweep-plan.json'), JSON.stringify(plan, null, 2));
  console.log(`Backline sweep complete. Estimated credits: ${estimatedCredits}. Reported credits: ${reportedCredits}.`);
}
main().catch(error => {
  ensureOutputDir();
  fs.writeFileSync(path.join(OUTPUT_DIR, 'firecrawl-backline-low-sweep-error.txt'), `${new Date().toISOString()}\n${error.stack || error.message}\n`);
  console.error(error);
  process.exit(1);
});
