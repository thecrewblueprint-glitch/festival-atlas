...
const OUTPUT_DIR = process.env.OUTPUT_DIR || 'firecrawl-output-power-refresh';
...
const department = {
  branchId: 'power',
  branchName: 'Power',
  queryTerms: 'temporary power generator distro cable vendor festival production 2026',
  note: 'Supplemental Power refresh sweep. Do not overwrite prior Power research.'
};
...
const plan = {
  ...
  batches: batchTargets.map(batch => ({
    batchId: `branch-research-refresh-${batch.suffix}-power`,
    branchId: department.branchId,
    branchName: department.branchName,
    theme: `${department.branchName}: ${batch.theme}`,
...
async function firecrawlSearch(query) {
  ...
  const res = await fetch('https://api.firecrawl.dev/v2/search', {
    ...
    body: JSON.stringify({ query, limit: MAX_SEARCH_RESULTS, sources: [{ type: 'web' }], country: 'US' })
  });
  let json;
...
async function researchTarget([opportunityId, opportunityName]) {
  ...
  try {
    ...
    record.searchResults = normalizeSearchResults(json).slice(0, MAX_SEARCH_RESULTS);
  } catch (error) {
...
function renderMarkdown(results) {
  const lines = [];
  lines.push('# Firecrawl Power Refresh Micro 20 Sweep', '', `Generated: ${results.generatedAt}`, 'Department: power', `Estimated credits: ${results.estimatedCredits}`, `Reported credits: ${results.reportedCredits}`, '');
  lines.push('## Rule', '', department.note, 'Search-only. No scrapes. Convert only reviewed public-safe findings into supplemental files.', '');
  for (const batch of results.batches) {
    lines.push(`## ${batch.batchId}`, '', `Theme: ${batch.theme}`, '');
    for (const target of batch.targets) {
      lines.push(`### ${target.opportunityName}`, '', `Opportunity ID: \`${target.opportunityId}\``, `Query: \`${target.query}\``, '');
      if (target.errors.length) lines.push('Errors:', ...target.errors.map(e => `- ${e}`), '');
      lines.push('Search results:');
      if (!target.searchResults.length) lines.push('- None returned.');
      target.searchResults.forEach(r => { lines.push(`- ${r.title || 'Untitled'} — ${r.url}`); if (r.description) lines.push(`  - ${r.description}`); });
      lines.push('', 'Scraped notes:', '- No pages scraped in micro mode.', '');
    }
  }
  lines.push('## Request log', '');
  requests.forEach(r => lines.push(`- search ${r.status} estimated=${r.estimatedCredits} reported=${r.reportedCredits} ${r.request.query}`));
  return lines.join('\n');
}
async function main() {
  ensureOutputDir();
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'firecrawl-power-refresh-results.json'), JSON.stringify(results, null, 2));
  fs.writeFileSync(path.join(OUTPUT_DIR, 'firecrawl-power-refresh-report.md'), renderMarkdown(results));
  fs.writeFileSync(path.join(OUTPUT_DIR, 'firecrawl-power-refresh-plan.json'), JSON.stringify(plan, null, 2));
  console.log(`Power refresh complete. Estimated credits: ${estimatedCredits}. Reported credits: ${reportedCredits}.`);
}
main().catch(error => {
  ensureOutputDir();
  fs.writeFileSync(path.join(OUTPUT_DIR, 'firecrawl-power-refresh-error.txt'), `${new Date().toISOString()}\n${error.stack || error.message}\n`);
  console.error(error);
  process.exit(1);
});
