...
const OUTPUT_DIR = process.env.OUTPUT_DIR || 'firecrawl-output-site-ops-refresh';
...
const department = {
  branchId: 'site-ops',
  branchName: 'Site Ops',
  queryTerms: 'site operations vendor fencing barricade toilets sanitation waste parking festival production 2026',
  note: 'Supplemental Site Ops refresh sweep. Do not overwrite prior Site Ops research.'
};
...
const plan = {
  ...
  batches: batchTargets.map(batch => ({ batchId: `branch-research-refresh-${batch.suffix}-site-ops`, branchId: department.branchId, branchName: department.branchName, theme: `${department.branchName}: ${batch.theme}`, targets: batch.targets }))
};
