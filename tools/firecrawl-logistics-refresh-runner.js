...
const OUTPUT_DIR = process.env.OUTPUT_DIR || 'firecrawl-output-logistics-refresh';
...
const department = {
  ...
  queryTerms: 'event logistics trucking freight shuttle parking traffic vendor festival production 2026',
  note: 'Supplemental Logistics refresh sweep. Do not overwrite prior Logistics research.'
};
...
const plan = {
  ...
  batches: batchTargets.map(batch => ({ batchId: `branch-research-refresh-${batch.suffix}-logistics`, branchId: department.branchId, branchName: department.branchName, theme: `${department.branchName}: ${batch.theme}`, targets: batch.targets }))
};
