(function(){
  ...
  function combinedBar(event,weekStart,weekEnd,row){
    ...
    return '<button class="'+cls+'" type="button" style="grid-column:'+colStart+' / '+colEnd+';grid-row:'+row+'" onclick="event.stopPropagation();openOpportunity(\''+esc(event.id)+'\')" title="'+esc(event.name)+'">'+showHtml+'<span class="cal-work-label">Approx.</span></button>';
  }
  ...
  function installStyles(){
    ...
    style.textContent=''+
      ...
      '.cal-combined-bar{pointer-events:auto;position:relative;display:flex;align-items:center;justify-content:flex-start;min-width:0;border:1px dashed rgba(127,183,255,.78);background:rgba(127,183,255,.12);color:#d9ebff;border-radius:8px;padding:3px 8px;font-size:.7rem;font-weight:850;cursor:pointer;box-shadow:0 4px 10px rgba(0,0,0,.16);overflow:hidden}.cal-combined-bar:hover{filter:brightness(1.16)}.cal-combined-bar.continues-in{border-top-left-radius:0;border-bottom-left-radius:0}.cal-combined-bar.continues-out{border-top-right-radius:0;border-bottom-right-radius:0}'+
      '.cal-work-label{position:relative;z-index:2;margin-left:auto;color:#9dc8ff;font-size:.62rem;text-transform:uppercase;letter-spacing:.06em;opacity:.72}'+
      '.cal-show-overlay{position:absolute;z-index:1;top:4px;bottom:4px;border-radius:999px;border:1px solid rgba(242,183,5,.86);background:linear-gradient(90deg,rgba(242,183,5,.78),rgba(217,148,0,.62));box-shadow:0 0 0 1px rgba(0,0,0,.12);display:flex;align-items:center;justify-content:center;padding:0 8px;min-width:26px}'+
