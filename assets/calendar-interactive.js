(function(){
  ...
  function combinedBar(event,weekStart,weekEnd,row){
    ...
    if(intersects(sr,outer.start,outer.end)){
      ...
      showHtml='<i class="cal-show-overlay" style="left:'+left+'%;width:'+width+'%"></i>';
    }
    return '<button class="'+cls+'" type="button" style="grid-column:'+colStart+' / '+colEnd+';grid-row:'+row+'" onclick="event.stopPropagation();openOpportunity(\''+esc(event.id)+'\')" title="'+esc(event.name)+'">'+showHtml+'<span>'+esc(event.name)+'</span><small>'+durationDays(sr)+'d show</small></button>';
  }
  ...
  function installStyles(){
    ...
    style.textContent=''+
      ...
      '.cal-combined-bar{pointer-events:auto;position:relative;display:flex;align-items:center;justify-content:space-between;gap:8px;min-width:0;border:1px dashed rgba(127,183,255,.78);background:rgba(127,183,255,.12);color:#d9ebff;border-radius:8px;padding:3px 8px;font-size:.7rem;font-weight:850;cursor:pointer;box-shadow:0 4px 10px rgba(0,0,0,.16);overflow:hidden}.cal-combined-bar:hover{filter:brightness(1.16)}.cal-combined-bar span,.cal-combined-bar small{position:relative;z-index:2}.cal-combined-bar span{overflow:hidden;white-space:nowrap;text-overflow:ellipsis}.cal-combined-bar small{font-weight:900;color:#fff3bf}.cal-combined-bar.continues-in{border-top-left-radius:0;border-bottom-left-radius:0}.cal-combined-bar.continues-out{border-top-right-radius:0;border-bottom-right-radius:0}'+
      '.cal-show-overlay{position:absolute;z-index:1;top:4px;bottom:4px;border-radius:999px;border:1px solid rgba(242,183,5,.86);background:linear-gradient(90deg,rgba(242,183,5,.72),rgba(217,148,0,.58));box-shadow:0 0 0 1px rgba(0,0,0,.12)}'+
