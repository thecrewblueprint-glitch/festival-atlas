(function(){
  ...
  function countBranchRecords(){
    return Object.keys(window).filter(function(k){return /^OPPORTUNITY_BRANCH_RESEARCH_BATCH_/.test(k)}).reduce(function(total,k){
      var d=window[k];
      return total+(d&&Array.isArray(d.targets)?d.targets.length:0);
    },0);
  }
  ...
  function render(){
    var el=$('#app');
    if(!el||!window.scopedOpportunities||!window.employers||!window.iatseLocals||!window.branches)return false;
    var stats=''+
      '<div class="stats">'+
        '<div class="stat"><b>'+window.scopedOpportunities.length+'</b><span>active opportunities</span></div>'+
        '<div class="stat"><b>'+window.employers.length+'</b><span>employer leads</span></div>'+
        '<div class="stat"><b>'+window.iatseLocals.length+'</b><span>IATSE records</span></div>'+
        '<div class="stat"><b>'+window.branches.length+'</b><span>production branches</span></div>'+
        '<div class="stat"><b>'+countBranchRecords()+'</b><span>branch records</span></div>'+ 
      '</div>';
    el.innerHTML=''+
      '<h2>Guide for Use</h2>'+stats+
      '<p class="lead">Production Atlas is a public-safe scouting dashboard for live-event work targets. Use it to identify events, venues, departments, vendor routes, employer leads, and local labor pathways worth researching. It is not a job board, a confirmed hiring list, or a private contact database.</p>'+ 
