(function(){
  ...
  function buildIndex(){
    ...
    Object.keys(window).forEach(function(key){
      ...
      dataset.targets.forEach(function(target){
        records.push(Object.assign({},target,{
          branchId:target.branchId||dataset.branchId,
          branchName:target.branchName||dataset.branchName,
    ...
    return {
      ...
      records:records,
      byOpportunityBranch:records.reduce(function(map,record){
        var keys=[
          norm(record.opportunityId)+'::'+record.branchId,
          norm(record.opportunityName)+'::'+record.branchId
        ];
        keys.forEach(function(key){map[key]=record;});
        return map;
      },{})
    };
  }
  ...
  function findBranchRecord(opportunity,branchId){
    ...
    var keys=[
      norm(opportunity.id)+'::'+branchId,
      norm(opportunity.name)+'::'+branchId
    ];
    for(var i=0;i<keys.length;i++){
      if(index.byOpportunityBranch[keys[i]])return index.byOpportunityBranch[keys[i]];
    }
    var oppNorm=norm(opportunity.name);
    return (index.records||[]).find(function(record){
      return record.branchId===branchId&&(norm(record.opportunityName)===oppNorm||norm(record.opportunityId)===norm(opportunity.id));
    });
  }

  function employerLink(id){
    var list=window.RESOURCE_EMPLOYERS||[];
  ...
  function boot(){
    loadPackages().then(function(){
      ...
      var timer=setInterval(function(){
        installOverride();
        tries++;
        if(window.__branchResearchOpenOpportunityInstalled||tries>50)clearInterval(timer);
      },100);
    });
  }
