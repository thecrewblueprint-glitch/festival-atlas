...
function stateFromJurisdiction(j){
  const matches=states.filter(s=>j.includes(s));
  if(j.includes('Washington, DC'))matches.push('District of Columbia');
  return [...new Set(matches)];
}
window.IATSE_US_LOCAL_DIRECTORY={
  meta:{
    name:'United States IATSE Local Directory — partial research snapshot',
    snapshotAsOf:'2026-06-21',
    audited:'2026-09-07',
    scope:'United States, Puerto Rico, and U.S. Virgin Islands organizations represented in the retained snapshot',
    coverageStatus:'partial_non_exhaustive_snapshot',
    canonicalCurrentSource:source,
    sourceUrl:source,
    sourceNote:'Use the official IATSE local-union directory for current discovery. This retained snapshot is an initial research index, not a complete directory, job board, membership decision, employer list, or jurisdiction ruling.',
    knownGapStates:['DE','ID','NH','ND','SD'],
    officialCountObservation:{observed:'2026-09-07',unitedStatesFinalCheck:305,unitedStatesSameDayConflictingObservation:306,canadaFinalCheck:40,treatment:'dynamic_source_count_do_not_freeze_as_complete'},
    recordCount:raw.trim().split('\n').length
  },
  locals:raw.trim().split('\n').map(line=>{const [local,district,jurisdiction,craft]=line.split('|');return{local,district,jurisdiction,craft,states:stateFromJurisdiction(jurisdiction),sourceUrl:source,verificationStatus:'partial_snapshot_verify_current_official_source'}})
};
})();
