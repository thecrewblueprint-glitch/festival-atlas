...
const rawEmployers=`Crew One Productions|Production labor staffing|Multi-market US
Rhino Staging|Production labor staffing|Multiple US markets
Bigger Hammer Production Services|Production labor staffing|Western US / Southern California
Showcall|Production labor staffing|Southeast / touring markets
Pangea Productions|User-supplied company lead|Identity not yet confirmed
Christie Lites|Lighting rental / touring production|North America
4Wall Entertainment|Lighting / video / production services|National
Bandit Lites|Lighting rental / touring production|National / touring
Upstaging|Lighting / transport / touring production|National
PRG|Full-service production technology|National / global
Solotech|Full-service production technology|National / global
Clair Global|Audio / communications / production services|National / global
Eighth Day Sound|Audio production|National / global
Firehouse Productions|Audio production|National / touring
LD Systems|Audio / lighting / video production|Texas / national projects
Onstage Systems|Full-service event production|Texas / regional touring
Brown Note Productions|Audio / lighting / video production|Colorado / national projects
Special Event Services|Audio / production services|Southeast / national touring
LMG|Audio / video / lighting production|National
Creative Technology|Video / display / production technology|National / global
Fuse Technical Group|Video / media servers / technical production|National
NEP Live Events|Broadcast / video production|National / global
TAIT|Staging / automation / scenic engineering|National / global
Mountain Productions|Staging / structures / rigging|North America
Stageco US|Staging / structures|US / global
All Access Staging & Productions|Staging / scenic / special structures|National
...
Freeman|Exhibitions / brand experience|National
GES|Exhibitions / live events|National
Shepard|Exhibitions / event services|National
Encore|Venue AV / event technology|National
IATSE Local Unions|Union stagehand labor|Jurisdiction-based nationwide
C3 Presents|Festival producer / promoter|National
Goldenvoice|Festival producer / promoter|California / national
Insomniac|Festival producer / promoter|National
Danny Wimmer Presents|Festival producer|National
...
Aggreko|Power / HVAC|National
United Rentals|Equipment rental|National
Sunbelt Rentals|Equipment rental|National
ASM Global venues|Venue operator|National
Oak View Group venues|Venue operator|National
Local production managers|Referral network|Market-specific
Venue crew pools|Venue labor|Market-specific`;
const west=['CA','NV','AZ','WA','OR','ID','UT','CO','NM','MT','WY'];
const mid=['IL','WI','MI','OH','IA','MO','MN','IN','ND','SD','NE','KS'];
const south=['TX','FL','GA','TN','KY','LA','AL','NC','SC','VA','OK','AR','MS','MD','DC','DE','WV'];
const ne=['NY','NJ','MA','RI','PA','CT','VT','NH','ME'];
...
const festivals=rawFestivals.trim().split('\n').map(line=>{let [name,city,state,month,producer]=line.split('|'); return {name,city,state,region:region(state),month:+month,window:'research calendar target',producer,scale:'research target',departments:festivalDeps(name),employerNotes:'Map stage builder, rigging, lighting, audio, video/LED, power, site ops, logistics, scenic/backline, stage management, production office, IATSE local, nonunion labor companies, and production company.',verificationStatus:'needs verification',source:'restored-expanded-dataset'};});
const employers=rawEmployers.trim().split('\n').map(line=>{let [name,type,reg]=line.split('|'); return {name,type,region:reg,departments:employerDeps(type,name),pathway:'verify application/referral route',notes:'Employer/vendor lead. Verify current market, department scope, classification, pay route, and festival relationship.',verificationStatus:'needs verification',source:'restored-expanded-dataset'};});
window.ATLAS_DATA={meta:{name:'Festival Atlas Research Dataset',branch:'research-version',updated:'2026-06-21',status:'research leads; verify before outreach',note:'Restored original 48 festival records plus expanded research targets. Older research files are preserved. Verify all vendor/employer relationships before treating them as current.',festival_count:festivals.length,employer_count:employers.length,original_uploaded_festival_count:48,original_uploaded_employer_count:45},departments,festivals,employers};
})();